...
Bootstrap создает новые базы в UTF-8 (`template0`, `LC_COLLATE=C`, `LC_CTYPE=C`), а все libpq/ODB подключения открываются с `client_encoding=UTF8`. На Windows это защищает JSON API от кластеров PostgreSQL, инициализированных в `WIN1251`: русские значения из SQL-дампа возвращаются клиенту как валидный UTF-8.
