...
/// Handler таблицы unit.
class UnitHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit UnitHandler(fasc::server::controllers::http::UnitHttpController& controller);
...
private:
  fasc::server::controllers::http::UnitHttpController& controller_;
};

/// Registers HTTP routes for the Unit table.
