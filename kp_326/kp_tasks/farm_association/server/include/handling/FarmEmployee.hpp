...
/// Handler таблицы farm_employee.
class FarmEmployeeHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit FarmEmployeeHandler(fasc::server::controllers::http::FarmEmployeeHttpController& controller);
...
private:
  fasc::server::controllers::http::FarmEmployeeHttpController& controller_;
};

/// Registers HTTP routes for the FarmEmployee table.
