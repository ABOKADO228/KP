...
/// Handler таблицы farm_ownership.
class FarmOwnershipHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit FarmOwnershipHandler(fasc::server::controllers::http::FarmOwnershipHttpController& controller);
...
private:
  fasc::server::controllers::http::FarmOwnershipHttpController& controller_;
};

/// Registers HTTP routes for the FarmOwnership table.
