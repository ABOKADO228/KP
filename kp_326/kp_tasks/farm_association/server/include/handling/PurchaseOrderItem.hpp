...
/// Handler таблицы purchase_order_item.
class PurchaseOrderItemHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit PurchaseOrderItemHandler(fasc::server::controllers::http::PurchaseOrderItemHttpController& controller);
...
private:
  fasc::server::controllers::http::PurchaseOrderItemHttpController& controller_;
};

/// Registers HTTP routes for the PurchaseOrderItem table.
