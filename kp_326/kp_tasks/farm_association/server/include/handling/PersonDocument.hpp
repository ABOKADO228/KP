...
/// Handler таблицы person_document.
class PersonDocumentHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit PersonDocumentHandler(fasc::server::controllers::http::PersonDocumentHttpController& controller);
...
private:
  fasc::server::controllers::http::PersonDocumentHttpController& controller_;
};

/// Registers HTTP routes for the PersonDocument table.
