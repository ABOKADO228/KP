...
/// Handler таблицы farm.
class FarmHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit FarmHandler(fasc::server::controllers::http::FarmHttpController& controller);
...
private:
  fasc::server::controllers::http::FarmHttpController& controller_;
};

/// Registers HTTP routes for the Farm table.
