...
/// Handler таблицы farm_role.
class FarmRoleHandler {
public:
  /// Создает handler таблицы.
  ///@param controller HTTP controller таблицы.
  explicit FarmRoleHandler(fasc::server::controllers::http::FarmRoleHttpController& controller);
...
private:
  fasc::server::controllers::http::FarmRoleHttpController& controller_;
};

/// Registers HTTP routes for the FarmRole table.
