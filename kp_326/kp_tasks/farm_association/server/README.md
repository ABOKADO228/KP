...
Новые базы создаются bootstrap-ом в UTF-8, а libpq/ODB подключения всегда используют `client_encoding=UTF8`. Поэтому даже Windows PostgreSQL-кластер с локалью `WIN1251` отдает русские значения в JSON API как валидный UTF-8.
