...
Текущий этап уже содержит общий CRUD-каркас и маршруты для всех таблиц предметной области из дампа. Клиентский auth-контракт синхронизирован с серверным `login`/`role`, управление пользователями защищено JWT ролей `agriculture_admin` и `association_director`, а ресурсные таблицы поддерживают клиентский поиск и фильтрацию уже загруженных строк. Дальнейший порядок работ:
...
3. Расширить бизнес-сценарии поверх базового CRUD: отчеты, согласования, сохраненные представления фильтров и связанные операции.
